...
let webSocket = new WebSocket('ws://localhost:8080/Five/lobby/'+loginid);
let inroomno = 0 ;
let bangjang = 0 ;
let receivedmsg = "";
let mypage = 1 ;
...
////////////////////////소켓으로부터 메시지 받고나서 실행되는 메소드들 //////////////////
	function showmember(){ // 접속 유저 표시하기	
		...
		for(let i = 0 ; i < receivedmsg.length ; i++){
			html +=  '<tr>'+
			'				<td>'+
			'				<button class="btn" onclick="infofo(\''+receivedmsg[i]["loginid"]+'\')"> <img alt="" src="/Five/img/profile/'+receivedmsg[i]["profile"]+'" style="width: 30px " class="rounded-circle">     '+receivedmsg[i]["nick"]+'</button>'+
			'				</td>'+
			'				</tr>';
		}
