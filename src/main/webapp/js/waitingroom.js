...
let webSocket = new WebSocket('ws://localhost:8080/Five/waitingroom/'+loginid +'/' + roomno);
let inroomno = roomno ;
let bangjang = 0 ;
let ready = 0 ;
let myready = 0 ;
...
/*    webSocket.onclose = function(event) {
	...
	window.onbeforeunload = function() {
		if(movecheck==1){
			location.href="/Five/game/gameboard.jsp?groom_no="+roomno;
		}
		else{
			roomout(roomno);
			...
			if(result[i]["loginid"]==loginid){ // 자기자신일경우
				if(bangjang==1){ // 방장일경우
					html +=
						...
						'<img alt="" src="/Five/img/profile/'+result[i]["profile"]+'" width="100%" height="220px" class="rounded-circle my-3">'+
...
function roomout(roomno){
	$.ajax({
		url : '/Five/lobby/roomout',
		data : {"roomno" : roomno},
		success : function(result){
			if(result == 1){
				$.ajax({
					url : '/Five/lobby/inuserlist',
					data : {"roomno" : roomno},
